...
export async function tick() {
  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      // campaign-wide daily count (in the campaign's own timezone day)
      const sentToday = Number((await pool.query(
        ...
        [campaign.id, campaign.timezone || 'UTC'])).rows[0].count);
      ...
      for (const account of accounts) {
        ...
        if (sent) {
          remaining--;
          // human-like randomized gap before this inbox sends again: 3-12 min
          const gapMin = 3 + Math.random() * 9;
          await pool.query(
...
export async function planCampaign(campaignId) {
  ...
  for (const lead of leads) {
    ...
    await pool.query(`UPDATE leads SET assigned_account_id=$1 WHERE id=$2 AND assigned_account_id IS NULL`,
      [acct, lead.id]);
  ...
  return { leads: leads.length, planned };
}
