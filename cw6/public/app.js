...
let state = { view:'campaigns', campaignId:null, ctab:'analytics', uniCat:'primary', uniFilter:{type:'all'}, camp:null };
...
// CAMPAIGN DETAIL
// ============================================================================
async function openCampaign(id){ state.campaignId=id; state.ctab='analytics'; await refreshCampaign(); }
...
async function runDiagnose(){
  ...
  try{
    ...
    <div class="step-card"><div class="step-head"><b>Repair the queue</b></div>
      <span class="muted">Re-points every queued email at the inboxes currently selected in Options. Fixes a queue stranded by changing inboxes after Start.</span>
      <div class="tabbar-row" style="margin-top:14px"><span></span><button class="primary-btn" id="rp-btn" onclick="repairQueue()">Repair queue</button></div>
      <div class="mini-msg" id="rp-msg"></div></div>
...
async function repairQueue(){
  const btn=$('rp-btn'); btn.disabled=true; btn.textContent='Repairing…';
  try{
    const d=await api('/campaigns/'+state.campaignId+'/repair',{method:'POST'});
    const m=$('rp-msg');
    if(d.ok){ m.style.color='var(--green)';
      m.textContent='Reassigned '+d.repaired+' email(s) to selected inboxes · '+(d.leadsReassigned||0)+' lead(s) re-homed · '+(d.pulledForward||0)+' pulled forward to now · '+(d.unstuckLeads||0)+' lead status reset. Re-run Diagnose.'; }
    else { m.style.color='var(--orange)'; m.textContent=d.error; }
  }catch(e){ $('rp-msg').textContent='Could not reach backend: '+e.message; }
  btn.disabled=false; btn.textContent='Repair queue';
}
...
function renderCtab(){
  ...
  if(state.ctab==='analytics') return renderAnalyticsTab(b);
...
// ---- ANALYTICS ----
async function renderAnalyticsTab(b){
  b.innerHTML='<div class="loading">Loading analytics…</div>';
  try{
    const d=await api('/campaigns/'+state.campaignId+'/analytics');
    if(!d.ok){ b.innerHTML='<div class="loading">'+esc(d.error)+'</div>'; return; }
    const t=d.totals;
    const card=(label,val,sub)=>`<div class="stat"><div class="stat-l">${label}</div><div class="stat-n">${val}</div>${sub?`<div class="stat-s">${sub}</div>`:''}</div>`;
    const max=Math.max(1,...d.series.map(s=>s.sent));
    const chart=`<div class="chart"><div class="chart-bars">${d.series.map(s=>`
        <div class="cbar" title="${esc(s.label)}: ${s.sent} sent, ${s.replies} replies">
          <div class="cbar-fill" style="height:${Math.round(s.sent/max*100)}%"></div>
          ${s.replies?`<div class="cbar-rep" style="height:${Math.round(s.replies/max*100)}%"></div>`:''}
        </div>`).join('')}</div>
      <div class="chart-x"><span>${esc(d.series[0]?.label||'')}</span><span>${esc(d.series[d.series.length-1]?.label||'')}</span></div>
      <div class="chart-key"><i class="k-sent"></i>Sent <i class="k-rep"></i>Replies</div></div>`;
    b.innerHTML=`
      <div class="stat-grid">
        ${card('Sequence started', t.started, t.leads+' total leads')}
        ${card('Emails sent', t.sent, t.sentToday+' today')}
        ${card('Replied', t.replied, t.replyRate+'%')}
        ${card('Bounced', t.bounced, t.bounceRate+'%')}
        ${card('Queued', t.queued, t.failed+' failed')}
      </div>
      <div class="step-card" style="max-width:none">
        <div class="step-head"><b>Last 30 days</b></div>${chart}</div>
      <div class="step-card" style="max-width:none"><div class="step-head"><b>By inbox</b><span class="muted">limit ${d.campaign.per_inbox_daily_limit}/day each · ${d.campaign.send_gap_minutes} min gap</span></div>
        <div class="table-head mini-grid"><span>Inbox</span><span>Sent today</span><span>Total sent</span><span>Queued</span><span>Next send</span></div>
        ${d.inboxes.map(i=>`<div class="mini-row mini-grid"><span>${esc(i.email)}</span><span>${i.sent_today} / ${d.campaign.per_inbox_daily_limit}</span><span>${i.total_sent}</span><span>${i.queued}</span><span class="muted">${new Date(i.next_send_at)<=new Date()?'ready':new Date(i.next_send_at).toLocaleTimeString()}</span></div>`).join('')||'<div class="loading">No inboxes selected.</div>'}
      </div>
      <div class="step-card" style="max-width:none"><div class="step-head"><b>By step</b></div>
        <div class="table-head mini-grid4"><span>Step</span><span>Sent</span><span>Queued</span><span>Failed</span></div>
        ${d.steps.map(s=>`<div class="mini-row mini-grid4"><span>Step ${s.step_number}</span><span>${s.sent}</span><span>${s.pending}</span><span>${s.failed}</span></div>`).join('')}
      </div>
      ${d.failures.length?`<div class="step-card" style="max-width:none;border-color:#6e3a1b"><div class="step-head"><b>Recent errors</b></div>
        ${d.failures.map(f=>`<div class="mini-row"><span class="muted">${f.n}×</span> ${esc(f.error)}</div>`).join('')}</div>`:''}`;
  }catch(e){ b.innerHTML='<div class="loading">Could not reach backend: '+esc(e.message)+'</div>'; }
}
...
async function renderOptionsTab(b,d){
  ...
  b.innerHTML=`<div class="step-card"><div class="step-head"><b>Accounts to send from</b></div>
    ...
    <div class="two-col">
      <label class="fld">Emails per inbox per day<input id="o-perinbox" type="number" min="1" max="500" value="${c.per_inbox_daily_limit||20}"></label>
      <label class="fld">Minutes between emails (per inbox)<input id="o-gap" type="number" min="1" max="240" value="${c.send_gap_minutes||3}"></label>
    </div>
    <span class="muted">Each inbox waits this long after every send, then emails the next lead. Inboxes are staggered so they don't fire together. ±10% jitter is added so sends don't look robotic.</span>
    <label class="fld" style="max-width:220px;margin-top:14px">Campaign-wide daily cap (safety ceiling)<input id="o-limit" type="number" min="1" max="5000" value="${c.daily_limit||30}"></label></div>
...
async function saveOptions(){
  ...
  try{
    ...
    const d2=await api('/campaigns/'+state.campaignId+'/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({
      ...
      per_inbox_daily_limit:+$('o-perinbox').value||20, send_gap_minutes:+$('o-gap').value||3 })});
