...
import { campaignAnalytics } from './analytics.js';
...
app.put('/campaigns/:id/settings', async (req, res) => {
  const { timezone, send_window_start, send_window_end, send_days, daily_limit,
          stop_on_reply, per_inbox_daily_limit, send_gap_minutes } = req.body;
  try {
    ...
    const pil = Math.max(1, Math.min(500, Number(per_inbox_daily_limit) || 20));
    const gap = Math.max(1, Math.min(240, Number(send_gap_minutes) || 3));
    const r = await pool.query(
      `UPDATE campaigns SET timezone=$1, send_window_start=$2, send_window_end=$3,
         send_days=$4, daily_limit=$5, stop_on_reply=$6,
         per_inbox_daily_limit=$7, send_gap_minutes=$8
       WHERE id=$9 RETURNING *`,
      [timezone, ws, we, send_days, dl, !!stop_on_reply, pil, gap, req.params.id]);
...
app.get('/campaigns/:id/analytics', async (req, res) => {
  try { res.json(await campaignAnalytics(req.params.id)); }
  catch (err) { res.status(500).json({ ok:false, error: err.message }); }
});
