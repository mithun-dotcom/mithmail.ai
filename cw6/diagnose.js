...
export async function diagnose(campaignId) {
  ...
  // 6. queued rows actually claimable by a selected inbox — with a full breakdown
  const bd = (await pool.query(
    `SELECT
       COUNT(*) FILTER (WHERE se.status='pending') AS pending,
       COUNT(*) FILTER (WHERE se.status='pending' AND se.scheduled_for > now()) AS future,
       COUNT(*) FILTER (WHERE se.status='pending' AND (se.account_id IS NULL
              OR se.account_id NOT IN (SELECT account_id FROM campaign_accounts WHERE campaign_id=$1))) AS wrong_inbox,
       COUNT(*) FILTER (WHERE se.status='pending' AND l.status NOT IN ('pending','in_sequence')) AS bad_lead_status
     FROM scheduled_emails se JOIN leads l ON l.id = se.lead_id
     WHERE se.campaign_id=$1`, [campaignId])).rows[0];

  const nextAt = (await pool.query(
    `SELECT MIN(scheduled_for) AS m FROM scheduled_emails
     WHERE campaign_id=$1 AND status='pending'`, [campaignId])).rows[0].m;

  const leadStatuses = (await pool.query(
    `SELECT l.status, COUNT(*) FROM scheduled_emails se JOIN leads l ON l.id=se.lead_id
     WHERE se.campaign_id=$1 AND se.status='pending' GROUP BY l.status`, [campaignId])).rows;
  ...
  const reasons = [];
  if (Number(bd.future) > 0) reasons.push(`${bd.future} scheduled in the FUTURE (earliest ${nextAt ? new Date(nextAt).toISOString() : '?'}) — press "Repair queue" to send them now`);
  if (Number(bd.wrong_inbox) > 0) reasons.push(`${bd.wrong_inbox} assigned to an inbox that is NOT selected (or NULL) — press "Repair queue"`);
  if (Number(bd.bad_lead_status) > 0) reasons.push(`${bd.bad_lead_status} whose LEAD status blocks sending (${leadStatuses.map(r=>r.status+': '+r.count).join(', ')}) — press "Repair queue" to reset them`);

  add('Queued emails are claimable right now', claimable > 0,
      claimable > 0 ? `${claimable} row(s) ready`
      : (reasons.length ? reasons.join('  ||  ')
         : `${bd.pending} pending but none claimable — cause unclear, send me this screen`));
